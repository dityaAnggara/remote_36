...
class report_purchase_bantu_xls(report_xls):
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ws = wb.add_sheet('Penerimaan Bahan Pembantu')
        baris = 0
        style = xlwt.XFStyle()
        style.alignment.wrap = 1
        for o in objects:
            i = 1
            ws.write_merge(baris, baris, 0, 8, 'Laporan Penerimaan Bahan Pembantu')
            baris += 1
            ...
            for line in plines:
                for line3 in line.picking_ids:
                    for line4 in line3.move_lines:
                        if line4.product_id and line4.product_id.categ_id and line4.product_id.categ_id.complete_name:
                            if 'Supporting' in line4.product_id.categ_id.complete_name:
                                if line4.state == 'done':
...
report_purchase_bantu_xls('report.purchase.bantu.xls', 'purchase.bantu')
